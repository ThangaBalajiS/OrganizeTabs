(function () {
    ...
    window.renderGroups = async function () {
        ...
        if (groupOrder.length) {
            ...
            groupOrder.forEach(function (item) {
                var selectedStyle = '';
                if (item === lStorage.selectedCategory) {
            ...
            setTimeout(function () {
                ...
                $('.group-options').on('click', function (e) {
                    ...
                    dropdownn.off();
                    dropdownn.on('click', async function (e) {
                        e.stopPropagation();
                        ...
                        if ($(e.target).closest('.group-dd-item').hasClass('rename')) {
                            ...
                            setTimeout(function () {
                                ...
                                $('#group-name-editor').on('blur', async function () {
                                ...
                                $('#group-name-editor').on('keyup', async function (e) {
                                    if (e.keyCode === 13) {
                            ...
                            swal({
                                ...
                                if (isConfirm) {
                                    ...
                                    if (lStorage.selectedCategory === dataId) {
                                        lStorage.selectedCategory = 'all';
                                    }
                ...
                (function () {
                    $('.group-item-mods').off();
                    $('.group-item-mods').on('click', async function (e) {
                        if (!$(e.target).hasClass('group-dd-item') && e.target.id !== 'group-name-editor') {
                            const currentStore = await window.helpers.getStore();
                            currentStore.selectedCategory = $(this).attr('data-id');
                            await window.helpers.setStore(currentStore);
                            await window.renderGroups();
                            await window.renderTabs();
                        }
    ...
    $('.add-group-btn').on('click', async function () {
        ...
        groups[tempId] = { name: 'Untitled', all: [], similar: {} };
        
        lStorage.group = groups;
        lStorage.groupOrder = groupOrder;
